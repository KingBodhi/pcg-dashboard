...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-[var(--pcg-black)]/95 backdrop-blur-sm border-b border-[var(--pcg-border)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-4">
                    ...
                    <div className="p-2">
                      {ecosystemProjects.map((project) => (
                        <a
                          key={project.id}
                          href={project.url}
                          target={project.id === "pcg" ? "_self" : "_blank"}
                          rel="noopener noreferrer"
                          onClick={() => setEcosystemOpen(false)}
                          className={`
                            flex items-start gap-3 p-3 rounded-lg transition-all
                            ${project.id === "pcg" ? "bg-[var(--bg-surface)]" : "hover:bg-[var(--bg-surface)]"}
                          `}
                        >
                          <div
            ...
            <Link href="/" className="flex items-center gap-3">
              ...
              <div className="hidden sm:block">
                <span className="text-lg font-bold text-gradient-emerald">PowerClub</span>
          ...
          <nav className="hidden md:flex items-center gap-6">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="text-[var(--text-secondary)] hover:text-[var(--pcg-emerald)] transition-colors text-sm font-medium"
              >
                {item.label}
              </Link>
            ))}
          ...
          <div className="hidden md:flex items-center gap-4">
            <a
              href="https://powerclubglobal.com/login"
              target="_blank"
              rel="noopener noreferrer"
              className="text-[var(--text-secondary)] hover:text-[var(--pcg-emerald)] transition-colors text-sm font-medium"
            >
          ...
          <button
            ...
            className="md:hidden p-2 text-[var(--pcg-emerald)]"
          >
            {mobileMenuOpen ? (
      ...
      {/* Mobile Navigation */}
      {mobileMenuOpen && (
        <div className="md:hidden bg-[var(--pcg-surface)] border-t border-[var(--pcg-border)]">
          <nav className="px-4 py-4 space-y-2">
            {navItems.map((item) => (
              <Link
                ...
                className="block py-2 text-[var(--text-secondary)] hover:text-[var(--pcg-emerald)] transition-colors"
              >
                {item.label}
              </Link>
            ))}
            <div className="pt-4 space-y-2">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="block py-2 text-[var(--text-secondary)] hover:text-[var(--pcg-emerald)]"
              >
                Sign In
              </a>
              <a
